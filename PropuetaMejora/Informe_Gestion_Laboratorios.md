...
El problema operativo: Con el paso del tiempo, el repositorio institucional acumula imágenes Docker de cursos antiguos o tecnologías en desuso, dificultando su administración y aumentando el consumo de almacenamiento. Sin embargo, archivar únicamente en función de la antigüedad es riesgoso: se puede archivar por error una imagen que un curso todavía necesita, o mantener durante años imágenes que nadie descarga hace semestres solo porque "aún no cumplen el plazo". Además, hoy no existe visibilidad clara de qué curso o proyecto depende de qué imagen, por lo que eliminar cualquiera implica un salto de fe.

La solución propuesta: Implementar un sistema de ciclo de vida automatizado basado en el uso real de cada imagen y en un mapa de dependencias imagen-curso/proyecto. El sistema clasificará periódicamente cada imagen en tres estados: Activa, En observación, y Candidata a archivado. Antes de archivar cualquier imagen, se generará automáticamente un reporte de impacto mostrando qué cursos la usaron históricamente, notificando al Responsable de Imágenes y al docente asociado. Las imágenes archivadas permanecerán disponibles durante un periodo de gracia antes de su eliminación definitiva, permitiendo su recuperación en caso de ser necesarias.

El beneficio organizativo: El repositorio se limpia según evidencia real de uso y no por una fecha arbitraria, reduciendo el riesgo de romper un curso activo por error y detectando imágenes "zombis" mucho más rápido que con un plazo fijo. Adicionalmente, este mecanismo alimenta directamente el indicador de "tasa de utilización de imágenes oficiales" ya definido en el documento base, optimizando el almacenamiento institucional y facilitando la búsqueda de imágenes vigentes para docentes y administradores.
